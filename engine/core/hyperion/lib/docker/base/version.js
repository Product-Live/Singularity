    ...
    obj.prototype = {
        _init: function() {
            ...
            return ((new mongo().connect('shared')).then(function(db) {
                return ((self._db = db.collection('hyperion_version')));
            }));
        },

        get: function(name) {
            return (this._init().then(function(db) {
                return (db.find({name: name}).toArray().then(function(res) {
                    if (res[0]) {
        ...
        update: function(name) {
            ...
            self._init().then(function(d) {
                return (db = d).find({name: name}).toArray();
            }, function() {
                ...
                if (res.length > 0) {
                    ...
                    return db.update({name: name}, {$set: {version: version}});
                } else {
                    ...
                    return db.insert({name: name, version: version});
                }
            }).then(function() {
